angular.module('operation', ['ui.bootstrap','operationLogService','tm.pagination']).controller('errorLofCtrl', function ($scope,operationLog) {
    ...
    $scope.url = exloreErrorLog + '?token=' + $scope.token;
    $scope.searchLog = function(){
        ...
        operationLog.queryErrorlog($scope.token, $scope.param).success(function (data) {
    ...
    $scope.clearLog = function(){
        operationLog.clearErrorLog($scope.token).success(function (data) {
            $scope.searchLog();
        }).error(function (data) {
            console.log(data);
        });
