angular.module('operation', ['ui.bootstrap','operationLogService','tm.pagination']).controller('debugCtrl', function ($scope,operationLog) {
    ...
    $scope.url = exploreDebugLog + '?token=' + $scope.token;
    ...
    $scope.searchLog = function(){
        ...
        if($scope.action == undefined || $scope.action == null || $scope.action == ""){}else{
            $scope.param.layer = $scope.action;
        }
        ...
        operationLog.queryDebuglog($scope.token, $scope.param).success(function (data) {
    ...
    $scope.clearLog = function(){
        operationLog.clearDebugLog($scope.token).success(function (data) {
            $scope.searchLog();
        }).error(function (data) {
            console.log(data);
        });
